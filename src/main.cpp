...
int main(int argc, const char *argv[]) {
  ...
  Uint64 window_flags = SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL;
  ...
  std::string title = std::string("OpenGL/Metal Renderer Interface")
                          .append(" - ")
                          .append(platform);

  SDL_Window *window = SDL_CreateWindow(title.c_str(), window_width,
  ...
  glm::mat4 projection_matrix =
      camera.get_projection_matrix((float)window_width / (float)window_height);
  ...
  while (is_running) {
    while (SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_EVENT_WINDOW_RESIZED) {
        ...
        projection_matrix = camera.get_projection_matrix((float)window_width /
                                                         (float)window_height);
      }
